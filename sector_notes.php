...
get_post_ifset("galaxy_name, sector, command, limit, sectornum, note_id, scanfrom, stype, owner, planets, port, fighters, torps, team, note, type, search, Del, sort, port_name");
...
$template_object->assign("l_sn_search", $l_sn_search);
$template_object->assign("l_sn_ally", $l_sn_ally);
$template_object->assign("l_sn_base", $l_sn_base);
$template_object->assign("l_sn_enemy", $l_sn_enemy);
$template_object->assign("l_sn_port", $l_sn_port);
$template_object->assign("l_sn_teambase", $l_sn_teambase);
$template_object->assign("l_sn_minefield", $l_sn_minefield);
$template_object->assign("l_sn_sg_entry", $l_sn_sg_entry);
$template_object->assign("l_sn_spysector", $l_sn_spysector);
$template_object->assign("l_sn_indy", $l_sn_indy);
$template_object->assign("l_wormhole", $l_wormhole);
...
if ($command==$l_sn_editnote )
{
	...
	,note_active_date='$stamp'
	,note_galaxy_name=" . $db->qstr($galaxy_name) . "
...
if (($command==$l_sn_view or $command=="addnote"))
{
	if ($command==$l_sn_view)
	{
		...
		if ($result->RecordCount())
		{
			...
			$galaxy_name = $result->fields['note_galaxy_name'];
			$dateposted = date($local_date_full_format, strtotime($result->fields['note_active_date']));
		...
		$notecount=1;
		$sector_id=$sector;
		$galaxy_name = $galaxyinfo['galaxy_name'];
		...
		if ($sectorinfo['port_type'] != "none")
		{
			...
			else if ($basename == "wormhole")
			{
				...
				$galaxy_id2 = $result2->fields['galaxy_id'];
				$result2 = $db->SelectLimit("SELECT galaxy_name FROM {$db_prefix}universe_galaxy WHERE galaxy_id='$galaxy_id2'", 1);
				db_op_result($result2,__LINE__,__FILE__);
				$galaxy_name2 = $result2->fields['galaxy_name'];
				$portname = $galaxy_name2 . " " .$l_wormhole;
			}
			else
			{
		...
		$sector_port = $portname;
		$dateposted = date($local_date_full_format, strtotime($stamp));
	}

	$template_object->assign("l_sn_notedate", $l_sn_notedate);
	$template_object->assign("dateposted", $dateposted);
	$template_object->assign("shipgalaxyname", $galaxyinfo['galaxy_name']);
	...
	$template_object->assign("galaxy_name", $galaxy_name);
	...
	$template_object->display("master_template/sector_notes_view.tpl");
}else{
	if ($playerinfo[team] > 0){
	...
	if(!empty($sort))
	{
		...
		elseif($sort == "galaxyname")
		{
			$query .= " note_galaxy_name ASC, note_sector_name ASC";
		}
		elseif($sort == "date")
		{
			$query .= " note_active_date ASC, note_galaxy_name ASC, note_sector_name ASC";
		}
		else
		{
	...
	while (!$result->EOF && $result)
	{
		...
		$galaxy_names[$notelistcount] = $result->fields['note_galaxy_name'];
		$dateposted[$notelistcount] = date($local_date_full_format, strtotime($result->fields['note_active_date']));
		$notelistcount++;
		$result->MoveNext();
	}
	...
	$template_object->assign("sector_mines", $sector_mines);
	$template_object->assign("sector_scanfrom", $sector_scanfrom);
	$template_object->assign("galaxy_name", $galaxy_names);
	$template_object->assign("notes_team", $notes_team);
	$template_object->assign("notelistid", $notelistid);
	$template_object->assign("notelistnote", $notelistnote);
	$template_object->assign("notelistdate", $notelistdate);
	...
	$template_object->assign("teamnotelistdate", $teamnotelistdate);
	$template_object->assign("gotomain", $l_global_mmenu);
	$template_object->display("master_template/sector_notes_all.tpl");
}
