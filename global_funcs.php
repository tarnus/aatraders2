...
function checklogin()
{
	...
	global $db, $db_prefix, $playerinfo, $shipinfo, $sectorinfo, $galaxyinfo, $shipdevice, $shipcommodities, $galaxyinfo; 
	global $enable_spies, $response, $player_onlinetime_left, $playeroutoftime;
...
//echo print_r($_SESSION) . "<br>";
	if(!array_key_exists ('session_player_id', $_SESSION))
	{
		if($noreturn != 1)
		{
...
//		echo rawurldecode($_SERVER['PHP_SELF']) . "</br>";
	...
	$result2 = $db->SelectLimit("SELECT {$db_prefix}universe.*, {$db_prefix}universe_galaxy.galaxy_name FROM {$db_prefix}universe, {$db_prefix}universe_galaxy WHERE {$db_prefix}universe.sector_id='$shipinfo[sector_id]' and {$db_prefix}universe_galaxy.galaxy_id={$db_prefix}universe.galaxy_id", 1);
	db_op_result($result2,__LINE__,__FILE__);
	$sectorinfo = $result2->fields;
	$result2->close();

	$result2 = $db->SelectLimit("SELECT * FROM {$db_prefix}universe_galaxy WHERE galaxy_id='$sectorinfo[galaxy_id]'", 1);
	db_op_result($result2,__LINE__,__FILE__);
	$galaxyinfo = $result2->fields;
	$result2->close();
...
function template_display($templatename, $templatefile)
{
	...
	{
		$template_object->display("master_template/" . $templatefile);
	}
}
