...
if (!empty($probeinfo))
{
	if ($shipinfo['sector_id'] == $probeinfo['sector_id'])
	{
		...
		if (empty($command))
		{
			...
			$template_object->assign("gotomain", $l_global_mmenu);
			$template_object->display("master_template/probes/probes_display.tpl");
			include ("footer.php");
			die();
		}
		...
		elseif ($probeinfo['owner_id'] == $playerinfo['player_id']  && $probeinfo['owner_id'] > 0)
		{
			...
			if ($command == "defenses"){
				...
				$template_object->assign("l_clickme", $l_clickme);
				$template_object->assign("gotomain", $l_global_mmenu);
				$template_object->display("master_template/probes/probes_defenses.tpl");
				include ("footer.php");
				die();
			}
			elseif ($command == "orders")
			{
				...
				$template_object->assign("gotomain", $l_global_mmenu);
				$template_object->display("master_template/probes/probes_orders.tpl");
				include ("footer.php");
				die();
			}
		...
		elseif ($command == "scan")
		{
			...
			else
			{
				...
				$template_object->assign("gotomain", $l_global_mmenu);
				$template_object->display("master_template/probes/probes_scan.tpl");
				include ("footer.php");
				die();
			}
		}
