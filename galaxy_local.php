...
$porttypes = array();
$porttypes_name = array();
$porttypes[0] = 'unknown';
$porttypes_name[0] = $l_unknown;
$porttypes[1] = 'none';
$porttypes_name[1] = $l_none;
$porttypes[2] = 'upgrades';
$porttypes_name[2] = $l_upgrade_ports;
$porttypes[3] = 'devices';
$porttypes_name[3] = $l_device_ports;
$porttypes[4] = 'spacedock';
$porttypes_name[4] = $l_spacedock;
$porttypes[5] = 'casino';
$porttypes_name[5] = $l_casino;
...
while (!$cargo_query->EOF) 
{
	...
	$porttypes_name[$typecount] = get_commodity_name($cargo_query->fields['classname']);
	$typecount++;
	$cargo_query->Movenext();
}

$template_object->assign("porttypes", $porttypes);
$template_object->assign("porttypes_name", $porttypes_name);
...
$query2 = "SELECT sector_name, spiral_arm, sector_id, port_type, zone_id, x, y, z,SQRT(POW(($sectorinfo[x]-x),2)+POW(($sectorinfo[y]-y),2)+POW(($sectorinfo[z]-z),2)) as sector_distance FROM {$db_prefix}universe where galaxy_id=" . $shipinfo['galaxy_id'] . " and SQRT(POW(($sectorinfo[x]-x),2)+POW(($sectorinfo[y]-y),2)+POW(($sectorinfo[z]-z),2)) < $distance and sector_id!=$shipinfo[sector_id] and sg_sector = 0  ORDER BY sector_distance ASC,sector_id DESC";
...
$template_object->display("master_template/galaxy_local.tpl");
include ("footer.php");
