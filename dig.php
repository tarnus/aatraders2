...
if(!$enable_dignitaries)
{
	...
	$template_object->display("master_template/dig-die.tpl");
	include("footer.php");
	die();
}
...
if($res->RecordCount())
{
	...
	if($res->RecordCount())
	{
		...
		while(!$res->EOF)
		{
			...
			$galaxylocation[$shipcount] = $galaxynames[$dig['galaxy_id']];
			$galaxylocationgalaxy[$shipcount] = $dig['galaxy_id'];
			$digcount++;
			$res->MoveNext();
		}
...
$template_object->assign("gotomain", $l_global_mmenu);
$template_object->display("master_template/dig.tpl");
