...
if (!$enable_spies) 
{
	...
	$template_object->display("master_template/spy-die.tpl");
	include ("footer.php");
	die();
}
...
//		$base_factor = ($planetinfo['base'] == 'Y') ? $basedefense : 0;
		...
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/spies/send.tpl");
	break;
	...
	case "comeback";   //GETTING your spy back from enemy planet
		...
		$template_object->assign("l_toplanetmenu", $l_toplanetmenu);
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/spies/comeback.tpl");
	break;
	...
	case "change":   //CHANGING your spy settings on enemy planet
		...
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/spies/change.tpl");
	break;
	...
	case "cleanup_planet":   // TRYING to find enemy spies on my planet
		...
		$template_object->assign("l_toplanetmenu", $l_toplanetmenu);
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/spies/cleanup_planet.tpl");
	break;
	...
	case "cleanup_ship":   // TRYING to find enemy spies on my ship
		...
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/spies/cleanup_ship.tpl");
	break;
	...
	case "detect":   //DETECTED data
		...
		if (!$res->RecordCount()) 
		{
			...
			$template_object->assign("l_spy_linkback", $l_spy_linkback);
			$template_object->assign("gotomain", $l_global_mmenu);
			$template_object->display("master_template/spy-die2.tpl");
			include ("footer.php");
			die();
		}
		...
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/spies/detect.tpl");
	break;
	...
	default:	// SHOWING a summary table of all spies
		...
		if ($by2 == 'planet')
		{
			$by22 = "{$db_prefix}planets.name asc, {$db_prefix}planets.sector_id asc, spy_id asc";
		}
		elseif ($by2 == 'id')
		{
			$by22 = "spy_id asc";
		...
		elseif ($by2 == 'percent')
		{
			$by22 = "spy_percent desc, {$db_prefix}planets.sector_id asc, {$db_prefix}planets.name asc, spy_id asc";
		}
		elseif ($by2 == 'move_type')
		{
			$by22 = "move_type asc, {$db_prefix}planets.sector_id asc, {$db_prefix}planets.name asc, spy_id asc";
		}
		elseif ($by2 == 'owner')
		{
			$by22 = "{$db_prefix}players.character_name asc, {$db_prefix}planets.sector_id asc, {$db_prefix}planets.name asc, spy_id asc";
		}
		else
		{
			$by22 = "{$db_prefix}planets.sector_id asc, {$db_prefix}planets.name asc, spy_id asc";
		}

		if ($by3 == 'spycnt')
		{
			$by33 = "spy_id_cnt ASC, {$db_prefix}planets.sector_id ASC";
		}
		elseif ($by3 == 'plnname')
		{
			$by33 = "{$db_prefix}planets.name ASC";
		}
		else
		{
			$by33 = "{$db_prefix}planets.sector_id ASC";
		}
		...
		if ($res->RecordCount())
		{
			...
			$galaxylocationships = array();
			$galaxylocationgalaxyships = array();
			$res = $db->Execute("SELECT {$db_prefix}universe.galaxy_id, {$db_prefix}universe.sector_name, {$db_prefix}spies.*, {$db_prefix}players.character_name, {$db_prefix}ships.name AS ship_name, {$db_prefix}ship_types.name AS c_name, UNIX_TIMESTAMP({$db_prefix}players.last_login) AS online 
			...
			if ($res->RecordCount()) 
			{
				...
				while (!$res->EOF) 
				{
					...
					$galaxylocationships[$enemyshipcount] = $galaxynames[$spy['galaxy_id']];
					$galaxylocationgalaxyships[$enemyshipcount] = $spy['galaxy_id'];
					$enemyshipcount++;
					$res->MoveNext();
				}
				$template_object->assign("galaxylocationgalaxyships", $galaxylocationgalaxyships);
				$template_object->assign("galaxylocationships", $galaxylocationships);
			...
			if ($res->RecordCount()) 
			{
				...
				while (!$res->EOF) 
				{
					...
					$galaxylocationplanets[$enemyplanetcount] = $galaxynames[$spy['galaxy_id']];
					$galaxylocationgalaxyplanets[$enemyplanetcount] = $spy['galaxy_id'];
					$enemyplanetcount++;
					$res->MoveNext();
				}
				$template_object->assign("galaxylocationgalaxyplanets", $galaxylocationgalaxyplanets);
				$template_object->assign("galaxylocationplanets", $galaxylocationplanets);
			...
			if ($res->RecordCount())
			{
				...
				while (!$res->EOF) 
				{
					...
					$galaxylocationownplanets[$ownplanetspycount] = $galaxynames[$spy['galaxy_id']];
					$galaxylocationgalaxyownplanets[$ownplanetspycount] = $spy['galaxy_id'];
					$ownplanetspycount++;
					$res->MoveNext();
				}
				$template_object->assign("galaxylocationgalaxyownplanets", $galaxylocationgalaxyownplanets);
				$template_object->assign("galaxylocationownplanets", $galaxylocationownplanets);
		...
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/spies/display.tpl");
	break;
}   //swich
