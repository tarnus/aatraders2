  ...
  <tr>
    <td bgcolor="#000000" valign="top" align="center" colspan=2>
		<table cellspacing = "0" cellpadding = "0" border = "0" width="100%">
			<TR>
				<TD><p><b>{$l_sector}:</b> {$destination}</p> 
		<form action=main.php method=post>
		{$l_chf_therearetotalfightersindest}<br>
		{$l_chf_therearetotalminesindest}<br>
		<INPUT TYPE=RADIO NAME=response CHECKED VALUE=retreat>{$l_chf_youcanretreat}<BR>
		<INPUT TYPE=RADIO NAME=response VALUE=fight>{$l_chf_inputfight}<BR>{$has_fighters}
{if $sector_zone_id != 2 && $has_fighters > 0}
