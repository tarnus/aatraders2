    	...
    	{else}
    		{if $shipinfo.$deviceclass[$key] != $devicemaximum[$key]}
    			{if $device_free[$key] != 0}
	    			<a href="#" onClick="MakeMax('{$device_form_name[$key]}', {$device_free[$key]}); countTotal(); return false;">{php}echo NUMBER($device_free[$key]); {/php}</a>
	    		{else}
	    			{php}echo NUMBER($devicemaximum[$key]); {/php}
	    		{/if}
	    	{/if}
    	{/if}
    </td>
