...
<TABLE BORDER="0" CELLSPACING="0" CELLPADDING="0" WIDTH="100%" bgcolor="#000000">
	...
	{foreach key=key value=item from=$lr_sector}
		<TR BGCOLOR="{$color}">
			{if $lr_returnlink[$key] == 0}
				<TD class="nav_title_12" align="center"><A class="mnu" HREF="main.php?move_method=warp&destination={$lr_sector[$key]|urlencode}">{$lr_zonetype[$key]}{$lr_sector[$key]}</A></TD>
			{else}
				<TD class="nav_title_12" align="center"><A class="mnu2" HREF="main.php?move_method=warp&destination={$lr_sector[$key]|urlencode}">{$lr_zonetype[$key]}{$lr_sector[$key]}</A></TD>
			{/if}
	...
	<tr>
		<td colspan="10"><br>{$l_lrs_scanedsector}<br><BR>{$l_lrs_click}<br><br>{$gotomain}<br><br></td>
	</tr>
</table>
