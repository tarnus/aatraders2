...
if ($playerinfo['team'] == 0)
{
	...
	$template_object->display("master_template/team-defensesdie.tpl");
	include ("footer.php");
	die();
}
...
$query = "SELECT {$db_prefix}universe.galaxy_id, {$db_prefix}universe.sector_name, {$db_prefix}planets.* FROM {$db_prefix}planets, {$db_prefix}universe WHERE {$db_prefix}planets.team=$playerinfo[team] and {$db_prefix}universe.sector_id ={$db_prefix}planets.sector_id";
...
if(!empty($sort))
{
	...
	elseif($sort == "fighter" || $sort == "sensors" || $sort == "beams" || $sort == "torp_launchers" ||
	  $sort == "shields" || $sort == "cloak" || $sort == "owner" || $sort == "base" || $sort == "jammer" || $sort == "galaxy_id")
	{
...
if($num_planets < 1)
{
	...
	$template_object->assign("gotomain", $l_global_mmenu);
	$template_object->display("master_template/team-defensesnone.tpl");
	include ("footer.php");
	die();
}
...
$template_object->assign("gotomain", $l_global_mmenu);
$template_object->display("master_template/team-defenses.tpl");
include ("footer.php");
?>