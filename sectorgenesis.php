...
function getsgcost($sector_id){
	...
	global $db, $db_prefix, $playerinfo, $shipinfo, $level_factor, $max_sglinks;
...
if ($playerinfo['turns'] < 1)
{
	...
	$template_object->display("master_template/sectorgenesisdie.tpl");
	include ("footer.php");
	die();
}
...
$sector_res = $db->SelectLimit("SELECT sector_id FROM {$db_prefix}universe WHERE galaxy_id=" . $shipinfo['galaxy_id'] . " and sector_name=" . $db->qstr($target_sector), 1);
...
if ($sglink==1){
	...
	$sector_type = $sectorinfo['sg_sector'];
	...
	if (($sector_type == 0) or (($sector_type != 0) and ($tlinks < $max_sglinks))){
		...
		if ($port_type > 30){
			$port="none";
			$debug_query = $db->Execute ("insert into {$db_prefix}universe (zone_id ,star_size,port_type,x,y,z,beacon, sg_sector, creation_date, creator_id, spiral_arm, created_from, galaxy_id)
			values
			(1,$random_star,'$port','$sector[x]','$sector[y]','$sector[z]','', $gateway_sector, '$stamp', $playerinfo[player_id], '$sector[spiral_arm]', $shipinfo[sector_id], $shipinfo[galaxy_id]) ");
		...
		$newsectorname = create_sector_number((string)$shipinfo['galaxy_id'] + chr(80 + $sectorspiral_arm) . (string)$random_star);
	...
	if(($sector_type != 0)and ($rslink==1)){
		...
		$res = $db->Execute("SELECT * from {$db_prefix}links where link_start=$shipinfo[sector_id] ");
		$tlinks = $res->RecordCount();
		$res1 = $db->SelectLimit("SELECT * from {$db_prefix}universe where galaxy_id = $shipinfo[galaxy_id] and sector_name =" . $db->qstr($target_sector), 1);
		...
		if($sgcost != 0){
			...
			$template_object->assign("gotomain", $l_global_mmenu);
			$template_object->display("master_template/sectorgenesis.tpl");
			include ("footer.php");
			die();
		}else{
			$template_object->assign("error_msg", str_replace("[limit]", "<font color=#00ff00><b>$max_sglinks</b></font>", $l_sgns_forbid2));
