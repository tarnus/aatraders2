...
include ("npc/npc_attack.inc.php");
get_post_ifset("ship_id");
*/
$title = $l_att_title;
 
 

function npc_attack_response($npc_ship, $player_ship){
global $db, $db_prefix;
 // get player Info (npc)  and target info (player)

$debug_query = $db->SelectLimit("SELECT * FROM {$db_prefix}players WHERE currentship=" . $db->qstr($npc_ship), 1);
...
$debug_query = $db->SelectLimit("SELECT * FROM {$db_prefix}ships WHERE ship_id=" . $db->qstr($npc_ship), 1);
db_op_result($debug_query,__LINE__,__FILE__);
...
$target_torps_left = $targetship['torps'];

$target_armor_left = $targetship['armor_pts'];

$target_fighters_left = $targetship['fighters'];
...
include ("npc/attack_exchange.inc");
...
$debug_query = $db->Execute("UPDATE {$db_prefix}ships SET armor_pts=GREATEST(armor_pts-$target_armor_died, 0), energy=GREATEST(energy-$target_energy_lost, 0), fighters=GREATEST(fighters-$target_fighters_died, 0), torps=GREATEST(torps-$target_torps_died, 0) WHERE ship_id=$targetship[ship_id]");
db_op_result($debug_query,__LINE__,__FILE__);
...
if(($attacker_armor_left < 1 and $attacker_shields_left < 1) or ($target_armor_left < 1 and $target_shields_left < 1))
{
	if ($target_armor_left < 1 and $target_shields_left < 1)
	{
		...
		adminlog("LOG0_ADMIN_COMBAT", "<font color=\"yellow\"><B>Salvage Values:</B></font><BR> Ship Value: " . NUMBER($ship_value) . " - $l_cmb_attacker_salvage : upgrade_cost = $upgrade_cost, upgrade_factor = $upgrade_factor<br>");
	...
	if ($attacker_armor_left < 1 and $attacker_shields_left < 1)
	{
		...
		adminlog("LOG0_ADMIN_COMBAT", "<font color=\"yellow\"><B>Salvage Values:</B></font><BR> Ship Value: " . NUMBER($ship_value) . " - $l_cmb_target_salvage : upgrade_cost = $upgrade_cost, upgrade_factor = $upgrade_factor<br>");
