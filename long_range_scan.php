...
$result = $db->Execute("SELECT {$db_prefix}universe.wormhole_destination, {$db_prefix}universe.zone_id, {$db_prefix}universe.port_type, {$db_prefix}universe.sector_name, {$db_prefix}links.* FROM {$db_prefix}links, {$db_prefix}universe WHERE {$db_prefix}links.link_start='$shipinfo[sector_id]' and {$db_prefix}universe.sector_id ={$db_prefix}links.link_dest  ORDER BY {$db_prefix}universe.sector_name ASC");
while (!$result->EOF)
{
	$row = $result->fields;
	// get number of sectors which can be reached from scanned sector
	$result2 = $db->Execute("SELECT * FROM {$db_prefix}links WHERE link_start='$row[link_dest]' and link_dest = '$shipinfo[sector_id]'");
	$lr_returnlink[] = $result2->RecordCount();

	$result2 = $db->Execute("SELECT * FROM {$db_prefix}links WHERE link_start='$row[link_dest]'");
	$num_links = $result2->RecordCount();

	$res_return = $db->Execute("SELECT {$db_prefix}universe.sector_name FROM {$db_prefix}sector_log, {$db_prefix}universe WHERE {$db_prefix}sector_log.player_id=$playerinfo[player_id] and ({$db_prefix}sector_log.source=$row[link_dest] or {$db_prefix}sector_log.destination=$row[link_dest]) and ({$db_prefix}universe.sector_id ={$db_prefix}sector_log.source or {$db_prefix}universe.sector_id ={$db_prefix}sector_log.destination)");
	if($res_return->RecordCount() > 0)
	{
		$beenthere = "*";
	}
	else
...
//	$query96 = $result2->fields;
	...
	if ($port_type == "wormhole")
	{
		$result2 = $db->SelectLimit("SELECT galaxy_id FROM {$db_prefix}universe WHERE sector_id='$row[wormhole_destination]'", 1);
	...
	{
		$portname = get_commodity_name($port_type);
	}

	log_scan($playerinfo['player_id'], $row['link_dest'],$row['zone_id']);

	$lr_zonetype[] = ($row['zone_id'] == 2) ? $beenthere . "F> " : $beenthere . "=> ";
...
$template_object->assign("lr_returnlink", $lr_returnlink);
...
$template_object->assign("l_lrs_scanedsector", $l_lrs_scanedsector);
...
$template_object->assign("gotomain", $l_global_mmenu);
$template_object->display("master_template/long_range_scan.tpl");
