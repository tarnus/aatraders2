...
$max_sector_defense_mines = NUM_PER_LEVEL(1) * $sector_mine_multiplier;
...
if ($res > 0)
{
	while (!$res->EOF)
	{
		...
		$max_sector_defense_mines += NUM_PER_LEVEL($res->fields['sector_defense_weapons']) * $sector_mine_multiplier;
		$res->MoveNext();
	}
}
...
if ($playerinfo['turns'] < 1)
{
	...
	$template_object->display("master_template/defense_deploy_die.tpl");
	include ("footer.php");
	die();
}
...
else
{
	...
	if (!isset($nummines) or !isset($numfighters))
	{
		...
		$l_defense_deploy_max_mines=str_replace("[amount]", $max_sector_defense_mines, $l_defense_deploy_max_mines);

		$template_object->assign("l_defense_deploy_max_mines", $l_defense_deploy_max_mines);
		...
		$template_object->assign("shiptorps", (max(0, $max_sector_defense_mines - $total_sector_mines) < $shipinfo['torps']) ? max(0, $max_sector_defense_mines - $total_sector_mines) : $shipinfo['torps']);
		...
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/defense_deploy.tpl");
		include ("footer.php");
		die();
	}
	else
	{
		...
		if ($nummines > max(0, $max_sector_defense_mines - $total_sector_mines))
		{
			$showmines = $l_defense_deploy_toomanymines;
			$nummines = 0;
		}
		else
	...
	$template_object->assign("gotomain", $l_global_mmenu);
	$template_object->display("master_template/defense_deploy_show.tpl");
	include ("footer.php");
	die();
}
