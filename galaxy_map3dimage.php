...
get_post_ifset("shipsector, arm, distance, galaxy_id");
...
$sql = "SELECT * from {$db_prefix}universe where galaxy_id=$galaxy_id order by sector_id DESC";
if($arm != ''){
	$sql = "SELECT * from {$db_prefix}universe where spiral_arm = $arm and galaxy_id=$galaxy_id order by sector_id DESC";
}
