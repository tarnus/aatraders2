...
if (!isset($defense_id))
{
	...
	$template_object->display("master_template/defense_modify_die.tpl");
	include ("footer.php");
	die();
}
...
switch($response)
{
	case "fight":
		...
		else
		{
			...
			$target_mines_used = 0;
			$attack_energy_used = 0;
			if($attack_beamtomine_dmg != 0)
			{
				$target_mine_hit_pts = $total_sector_mines * $mine_hit_pts;

				$attack_mine_failure = calc_failure($attack_beamtomine_dmg, $targetshipshields);
				$attack_beamtomine_dmg -= $attack_mine_failure[1];

				$attack_mine_damage = calc_damage($attack_beamtomine_dmg, $beam_damage_all, $attackerlowpercent, $shipinfo['beams'], $highcloak, $target_mine_hit_pts);
				$attack_energy_used = $attack_mine_damage[3];
			...
			$debug_query = $db->Execute ("UPDATE {$db_prefix}ships SET energy=GREATEST(energy-$attack_energy_used, 0) WHERE " .
	...
	case "retrieve":
		...
		if ($quantity > 0)
		{
			$debug_query = $db->Execute("UPDATE {$db_prefix}sector_defense SET quantity=GREATEST(quantity - $quantity, 0) WHERE " .
	...
	default:
		...
		$template_object->assign("l_md_consist", $l_md_consist);
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/defense_modify.tpl");
		include ("footer.php");
		die();
		break;
}
