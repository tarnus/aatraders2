...
$result = $db->Execute("SELECT * FROM {$db_prefix}traderoutes WHERE player_id=$playerinfo[player_id] and galaxy_id=$shipinfo[galaxy_id]");
...
if ($num_traderoutes == 0)
{
	...
	$template_object->display("master_template/traderoutes/traderoute_none.tpl");
	include ("footer.php");
	die();
}else{
	...
	while ($i < $num_traderoutes)
	{
		...
		$tradesource_commodity[$i] = get_commodity_name($traderoutes[$i]['source_commodity']);
		...
		$tradedestination_commodity[$i] = get_commodity_name($traderoutes[$i]['destination_commodity']);
		...
		if ($traderoutes[$i]['move_type'] == 'R')
		{
			...
			if ($traderoutes[$i]['roundtrip'] != 'Y' || $traderoutes[$i]['source_sector'] == $traderoutes[$i]['destination_sector'])
			{
				...
				$res = $db->Execute("SELECT * FROM {$db_prefix}autoroutes WHERE autoroute_id=" . $traderoutes[$i]['forward_autoroute_id']);
				db_op_result($res,__LINE__,__FILE__);
				if($res->RecordCount() > 0)
				{
					$list = explode("|", $res->fields['warp_list']);
					$trade_energyscoop[$i] += count($list);
				}
			}else{
				$trade_energyscoop[$i] = 4;
				$res = $db->Execute("SELECT warp_list FROM {$db_prefix}autoroutes WHERE autoroute_id=" . $traderoutes[$i]['forward_autoroute_id']);
				db_op_result($res,__LINE__,__FILE__);
				if($res->RecordCount() > 0)
				{
					$list = explode("|", $res->fields['warp_list']);
				...
				$res = $db->Execute("SELECT warp_list FROM {$db_prefix}autoroutes WHERE autoroute_id=" . $traderoutes[$i]['return_autoroute_id']);
				db_op_result($res,__LINE__,__FILE__);
				if($res->RecordCount() > 0)
				{
					$list = explode("|", $res->fields['warp_list']);
	...
	$template_object->assign("l_tdr_galaxytrades", $l_tdr_galaxytrades);

	$template_object->assign("l_sector", $l_sector);
	$template_object->assign("l_delete", $l_tdr_del);
	$template_object->assign("gotomain", $l_global_mmenu);
	$template_object->display("master_template/traderoutes/traderoute_listroutes.tpl");
	include ("footer.php");
}
