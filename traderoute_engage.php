...
function checksectordefense($sectorname){

	global $db, $db_prefix, $playerinfo;

	$hostile = 0;

	$sector_res = $db->SelectLimit("SELECT sector_id FROM {$db_prefix}universe WHERE sector_name=" . $db->qstr($sectorname), 1);
	db_op_result($sector_res,__LINE__,__FILE__);
	$sector_id = $sector_res->fields['sector_id'];

	$result98 = $db->SelectLimit("SELECT player_id FROM {$db_prefix}sector_defense WHERE sector_id = $sector_id AND player_id <> $playerinfo[player_id]", 1);
	db_op_result($result98,__LINE__,__FILE__);
	if (!$result98->EOF)
	{
		...
		db_op_result($nsresult,__LINE__,__FILE__);
...
//$db->debug=1;
	...
	if($traderoute['source_type'] != 'port'){
		if ($traderoute['source_type'] == 'planet')	// get data from planet table
		{
	...
	if ($traderoute['destination_type'] != 'port'){
		if (($traderoute['destination_type'] == 'planet'))	// get data from planet table
		{
	...
	{
		$dist['triptime'] = 1;
		$res = $db->Execute("SELECT * FROM {$db_prefix}autoroutes WHERE autoroute_id=" . $traderoute['forward_autoroute_id']);
		db_op_result($res,__LINE__,__FILE__);
		if($res->RecordCount() > 0)
		{
			$list = explode("|", $res->fields['warp_list']);
			$dist['triptime'] += count($list);
			...
			for($i = 0; $i < count($list); $i++)
			{
				...
				if(checksectordefense($list[$i]) == "1")
				{
					...
					$template_object->assign("l_autoroute_return", "");
	...
	if ($traderoute['source_type'] == 'port')
	{
		...
		if ($source['port_type'] != 'spacedock' && $source['port_type'] != 'casino' && $source['port_type'] != 'devices' && $source['port_type'] != 'upgrade')
		{
			...
			{
				$l_tdr_shiptoolarge = str_replace("[port_type]", get_commodity_name($source['port_type']), $l_tdr_shiptoolarge);
	...
	if ($traderoute['destination_type'] == 'port')
	{
		...
		if ($dest['port_type'] != 'spacedock' && $dest['port_type'] != 'casino' && $dest['port_type'] != 'devices' && $dest['port_type'] != 'upgrade')
		{
			if ($query97['zone_id'] == 2 && $query97['max_hull'] < $average_stats && $playerinfo['player_id'] > 3)
			{
				$l_tdr_shiptoolarge = str_replace("[port_type]", get_commodity_name($dest['port_type']), $l_tdr_shiptoolarge);
				$l_tdr_shiptoolarge = str_replace("[sector_name]", $query97['sector_name'], $l_tdr_shiptoolarge);
				traderoute_die($l_tdr_shiptoolarge);
			}
		}
