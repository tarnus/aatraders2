...
<?php if ($this->_vars['multiplepages'] != 0): ?>
	...
	for($i = 0; $i <= $multiplepages; $i++){
	 ...
	 ?>
	<option value="-1"><?php echo $this->_vars['l_all']; ?></option>
	</select>
...
<?php extract($this->_vars, EXTR_REFS);  
      $color="#3A3B6E";
		for($i = 0; $i < $rankcount; $i++){
			...
			echo "	<td nowrap align=center><img border=\"0\" src=\"templates/master_template/images/rank/" . $rankimage[$i] . "\" align=\"absmiddle\" alt=\"" . $rankimage_name[$i] . "\"></td>";
			...
			if($rankprofileid[$i] != 0){
				echo "<a href=\"http://www.aatraders.com?player_id=" . $rankprofileid[$i] . "\" target=\"_blank\"><b>" . $rankname[$i] . "</b></a>";
			}else{ 
				echo "<b>" . $rankname[$i] . "</b>";
			}
