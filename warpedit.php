...
if ($playerinfo['turns'] < 1)
{
	...
	$template_object->display("master_template/warpeditdie.tpl");
	include ("footer.php");
	die();
}
...
if(!isset($confirm)){
	...
	$template_object->assign("gotomain", $l_global_mmenu);
	$template_object->display("master_template/warpedit.tpl");

	include ("footer.php");
}else{
	$nameresult = $db->Execute("SELECT sector_id, galaxy_id FROM {$db_prefix}universe WHERE sector_name=" . $db->qstr($target_sector));
	$target_sector_id = $nameresult->fields['sector_id'];
	$target_galaxy_id = $nameresult->fields['galaxy_id'];

	if ($target_galaxy_id != $shipinfo['galaxy_id'])
	{
		$template_object->assign("error_msg", $l_warp_forbid);
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/warpedit_delete.tpl");
		include ("footer.php");
		die();
	}
	...
	if ($shipdevice['dev_warpedit']['amount'] < 1)
	{
		$template_object->assign("error_msg", $l_warp_none);
		$template_object->assign("gotomain", $l_global_mmenu);
		$template_object->display("master_template/warpedit_add.tpl");
		include ("footer.php");
		die();
	}
	...
	$template_object->assign("l_submit", $l_submit);
	$template_object->assign("gotomain", $l_global_mmenu);
	$template_object->display("master_template/warpedit_confirm.tpl");
