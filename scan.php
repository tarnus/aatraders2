...
$player_id = $targetshipinfo['player_id'];
...
if ($targetshipinfo['sector_id'] != $shipinfo['sector_id'] || $shipinfo['sector_id'] == 1)
{
	...
	$template_object->display("master_template/scandie.tpl");
	include ("footer.php");
	die();
}
...
{
	$fedcheckbounty = $l_by_nofedbountyship;
}
...
while(!$res->EOF)
{
	$cargo_name[$cargo_items] = get_commodity_name($res->fields['cargo_name']);
...
$template_object->display("master_template/scan.tpl");
