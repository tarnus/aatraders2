...
if ($shipdevice['dev_emerwarp']['amount'] > 0)
{
	...
	$findem = $db->SelectLimit("SELECT sector_id, sector_name FROM {$db_prefix}universe where sg_sector = 0 and sector_id > 3 and galaxy_id = " . $shipinfo['galaxy_id'] . " ORDER BY rand()", 1);
...
$template_object->display("master_template/emerwarp.tpl");
