...
$result3 = $db->Execute("SELECT sector_name, link_dest FROM {$db_prefix}links, {$db_prefix}universe WHERE {$db_prefix}links.link_start='$query96[sector_id]' and {$db_prefix}universe.sector_id ={$db_prefix}links.link_dest ORDER BY {$db_prefix}universe.sector_name ASC");
...
if ($result3 > 0)
{
	while (!$result3->EOF)
	{
		if($shipinfo['sector_id'] == $result3->fields['link_dest'])
		{
			$link_list .= "<font color=\"lime\">" . $result3->fields['sector_name'] . "</font>";
		}
		else
		{
			$link_list .= $result3->fields['sector_name'];
		}
		if ($i + 1 != $result3->RecordCount())
		{
			$link_list .= ", ";
		}
		$i++;
...
$template_object->assign("l_port", $l_port);
	...
	else
	{
		...
		{
			$result2 = $db->SelectLimit("SELECT galaxy_id FROM {$db_prefix}universe WHERE sector_id='$query96[wormhole_destination]'", 1);
			...
			$icon_alt_text = $galaxy_name . " " .$l_wormhole;
			$icon_port_type_name = $port_type;
		}
		else
		{
			$icon_alt_text = get_commodity_name($port_type);
			$icon_port_type_name = $port_type;
		}
	}
$galaxy_name2 = $galaxyinfo['galaxy_name'];
$template_object->assign("galaxy_name", $galaxy_name2);
...
$template_object->assign("gotomain", $l_global_mmenu);
$template_object->display("master_template/sector_scan.tpl");
